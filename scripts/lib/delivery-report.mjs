...
function gaps(snapshot, records) { const result=[]; for (const [name, source] of Object.entries(snapshot.sources)) if (source.status !== "available") result.push(`- ${name}: ${source.status} — ${source.reason}`); if (records.some((record) => record.release.status !== "shipped")) result.push("- Some explicit delivery records have no GitHub Release proven on the exact commit."); if (records.some((record) => record.deployment.status !== "passed")) result.push("- Deployment runtime proof is missing or not passed for some records; this does not change GitHub Release shipping truth."); return result.length ? result : ["- None."]; }
function recordLine(record) { return `- ${record.id}: Plane ${record.plane.status}; CI ${record.ci.status}; release ${record.release.status}; deployment ${record.deployment.status}.`; }
function decision({ shipped, deployed, ciPassed, records, overLimit, aging }) { if (!records.length) return "No decision-grade delivery records yet. Add explicit Plane-to-PR links."; if (overLimit.length || aging.length) return "Portfolio attention: reduce WIP or resolve aging work before adding parallel work."; if (shipped.length === records.length && ciPassed.length === records.length) return deployed.length === records.length ? "Shipping path healthy." : "Shipping proven. Runtime deployment evidence remains incomplete."; return "Do not claim complete shipping health; resolve the listed exact-evidence gaps."; }
