...
import { at, buildkite, plane, provider, releases } from "./helpers/delivery-evidence-fixture.mjs";
test("delivery join requires explicit Plane key and exact commit evidence",()=>{const x=joinDeliveryEvidence({providerSnapshot:provider(),planeSnapshot:plane(),buildkiteSnapshots:[buildkite()],releaseSnapshot:releases()});const r=x.deliveryRecords[0];assert.deepEqual(r.plane,{status:"linked",key:"INTB-260",stateGroup:"started",updatedAt:at});assert.equal(r.ci.status,"passed");assert.equal(r.release.status,"shipped");assert.equal(r.deployment.status,"unavailable");});
