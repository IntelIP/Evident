...
This bootstrap intentionally runs the repository's existing `npm run check`
only. It has no deployment, provider, release, or production-data credentials.
Future delivery-validation steps must remain bounded, exact-head, and separately
reviewed.
